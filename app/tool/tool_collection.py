...
class ToolCollection:
   ...
   # [新增] 实现 remove_tool 方法
    def remove_tool(self, name: str):
        """Remove a single tool from the collection by its name."""
        if name not in self.tool_map:
            # If tool doesn't exist, do nothing.
            return self

        # Remove from the dictionary
        del self.tool_map[name]

        # Recreate the tuple of tools without the removed one
